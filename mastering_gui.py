# mastering_gui.py (v3.0 - Final Local Version)
# This version simplifies the import statement to match the new engine filename.
...
# --- THIS IS THE FIX ---
# We are importing from the simply named 'audio_mastering_engine.py'
...
class MasteringApp(ThemedTk):
    def __init__(self):
        ...
        self.title("Audio Mastering Suite v3.0 (Local)")
        ...
        file_frame.grid(row=current_row, column=0, sticky="ew", pady=(0, 20)); current_row += 1
        ...
        preset_frame.grid(row=current_row, column=0, sticky="ew", pady=20); current_row += 1
        ...
        params_frame.grid(row=current_row, column=0, sticky="ew", pady=20); current_row += 1
        ...
        multiband_header_frame.grid(row=current_row, column=0, sticky="ew", pady=(20, 5)); current_row += 1
        ...
        self.multiband_controls_frame.grid(row=current_row, column=0, sticky="ew", pady=(0, 20)); current_row += 1
        ...
        art_frame.grid(row=current_row, column=0, sticky="ew", pady=20); current_row += 1
        art_frame.grid_columnconfigure(1, weight=1)
        ttk.Label(art_frame, text="AI Art Prompt (Optional):").grid(row=0, column=0, sticky='w')
        ttk.Entry(art_frame, textvariable=self.art_prompt).grid(row=0, column=1, sticky='ew', padx=10)
        
        # Process and Status
        process_frame = ttk.Frame(main_frame)
        process_frame.grid(row=current_row, column=0, sticky="ewns", pady=20); current_row += 1
        ...
        # Art Display
        self.art_display_frame = ttk.Frame(main_frame, relief=tk.SUNKEN)
        self.art_display_frame.grid(row=current_row, column=0, sticky="nsew", pady=20); current_row += 1
        self.art_display_frame.grid_columnconfigure(0, weight=1)
        self.art_display_frame.grid_rowconfigure(0, weight=1)
        main_frame.grid_rowconfigure(current_row - 1, weight=1)
        
        self.art_label = ttk.Label(self.art_display_frame, text="AI Art Will Appear Here", anchor="center")
    ...
    def apply_preset(self, preset_name):
        if preset_name == "None":
            self.bass_boost.set(0); self.mid_cut.set(0); self.presence_boost.set(0); self.treble_boost.set(0)
            return
        preset = EQ_PRESETS.get(preset_name)
        if preset:
            self.bass_boost.set(preset.get("bass_boost", 0)); self.mid_cut.set(preset.get("mid_cut", 0))
            self.presence_boost.set(preset.get("presence_boost", 0)); self.treble_boost.set(preset.get("treble_boost", 0))
    ...
    def select_input_file(self):
        ...
        if file_path:
            ...
            path, filename = os.path.split(file_path); name, ext = os.path.splitext(filename)
            self.output_file.set(os.path.join(path, f"{name}_mastered{ext}"))

    def select_output_file(self):
        file_path = filedialog.asksaveasfilename(filetypes=[("WAV file", "*.wav"), ("MP3 file", "*.mp3")], defaultextension=".wav")
    ...
    def start_processing_thread(self):
        ...
        settings = {
            ...
            "treble_boost": self.treble_boost.get(), "width": self.width.get(),
            "lufs": self.lufs.get(), "multiband": self.use_multiband.get(),
            "low_thresh": self.low_thresh.get(), "low_ratio": self.low_ratio.get(),
            "mid_thresh": self.mid_thresh.get(), "mid_ratio": self.mid_ratio.get(),
            "high_thresh": self.high_thresh.get(), "high_ratio": self.high_ratio.get(),
            "art_prompt": self.art_prompt.get(),
        }
        threading.Thread(target=process_audio, args=(settings, self.update_status, self.update_progress, self.update_art_display), daemon=True).start()
    ...
    def update_status(self, message):
        ...
        if "Success:" in message or "Error:" in message or "Failed:" in message:
            ...
            if "Success:" in message:
                clean_message = message.replace("Success: ", "")
                messagebox.showinfo("Success", f"{clean_message}\nYour audio file has been mastered successfully!")
    ...
    def update_art_display(self, image_path):
        ...
        try:
            ...
            frame_width = self.art_display_frame.winfo_width()
            frame_height = self.art_display_frame.winfo_height()
            if frame_width < 2 or frame_height < 2: frame_width, frame_height = 400, 400
            img_ratio, frame_ratio = img.width / img.height, frame_width / frame_height
